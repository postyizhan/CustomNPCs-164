...
**症状**：Stock Options 界面的 18 个 Max Stock 输入框和 Custom Reset Time 输入框显示为黑色矩形，无法看见文字，虽然能看到光标但输入任何字符都不显示。

**影响**：无法通过界面配置库存上限和自定义补货时间。需要通过其他方式（如 NBT 编辑器直接修改 NPC 数据）配置。
...
**根本原因**：未确定。SubGuiInterface 继承链的渲染机制与正常工作的 GuiNPCInterface2 有差异，可能与 Container GUI 上下文或 SubGui 的绘制顺序有关。

**临时方案**：
1. 用 NBT 编辑器（如 NBTExplorer）直接编辑 NPC 数据的 `Stock` 标签
2. 或在代码里预设默认库存配置后重新编译
