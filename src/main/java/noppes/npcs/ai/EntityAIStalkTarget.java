...
public class EntityAIStalkTarget extends EntityAIBase
{
    ...
    public void updateTask()
    {
    	...
    	if (this.theEntity.getNavigator().noPath() || this.overRide)
    	{
    		...
    		else if (this.targetEntity.canEntityBeSeen(theEntity))
    		{
    	...
    	if (!this.isLookingAway() && this.targetEntity.canEntityBeSeen(theEntity) && this.delay == 0)
    	{
    		this.overRide = true;
    		this.delay = 60;
    	}
    	...
    	for (int y = -2; y <= 2; y++)
    	{
    		for (int x = -radius; x <= radius; x++)
    		{
    			for (int z = -radius; z <= radius; z++)
    			{
    	            ...
    	            if (!this.theWorld.isAirBlock((int)j, (int)k, (int)l) && this.theWorld.isAirBlock((int)j, (int)k + 1, (int)l) && this.theWorld.isAirBlock((int)j, (int)k + 2, (int)l))
    	            {
