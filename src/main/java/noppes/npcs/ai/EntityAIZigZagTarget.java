        ...
        else
        {
        	...
        	if (pathentity != null)
        	{
        		if (pathentity.getCurrentPathLength() >= this.field_75426_g)
		        {
		        	...
		        	Vec3 vec3 = RandomPositionGenerator.findRandomTargetBlockTowards(this.theEntity, (int)field_75426_g, 3, this.theEntity.worldObj.getWorldVec3Pool().getVecFromPool(this.entityPosX, this.entityPosY, this.entityPosZ));
...
//		            	if (this.theEntity.getDistanceSq(vec3.xCoord, vec3.yCoord, vec3.zCoord) > field_75426_g * field_75426_g && this.targetEntity.getDistanceSq(vec3.xCoord, vec3.yCoord, vec3.zCoord) < this.targetEntity.getDistanceSq(entityPosX, entityPosY, entityPosZ))
    ...
    {
        return !this.theEntity.getNavigator().noPath() && this.targetEntity.isEntityAlive() && this.targetEntity.getDistanceSqToEntity(this.theEntity) > (double)(this.field_75426_g * this.field_75426_g) && this.theEntity.getEntitySenses().canSee(targetEntity);
    }
