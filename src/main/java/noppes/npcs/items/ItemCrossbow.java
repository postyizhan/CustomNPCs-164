...
public class ItemCrossbow extends ItemNpcInterface{
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World par2World, EntityPlayer player, int count) {
    	...
    	if(stack.stackTagCompound.getInteger("IsLoaded") == 1 || player.capabilities.isCreativeMode) {
    		...
    		EntityProjectile projectile = new EntityProjectile(player.worldObj, player, new ItemStack(Item.arrow, 1, 0), false);
    		...
    		if(!player.capabilities.isCreativeMode)
    			player.inventory.consumeInventoryItem(CustomItems.crossbowBolt.itemID);
    ...
    public void onUsingItemTick(ItemStack stack, EntityPlayer player, int count) 
    {
    	...
    	if(!player.capabilities.isCreativeMode){
	    	if(stack.stackTagCompound.getInteger("Reloading") == 1 && player.inventory.hasItem(CustomItems.crossbowBolt.itemID)){
	    		if(ticks == 20){
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
    {
		...
		if(!player.capabilities.isCreativeMode && player.inventory.hasItem(CustomItems.crossbowBolt.itemID) && stack.stackTagCompound.getInteger("IsLoaded") == 0)
