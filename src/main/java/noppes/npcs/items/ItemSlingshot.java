...
public class ItemSlingshot extends ItemNpcInterface{
    ...
    public void onPlayerStoppedUsing(ItemStack par1ItemStack, World worldObj, EntityPlayer player, int par4) {
        ...
        if (!player.capabilities.isCreativeMode && !player.inventory.consumeInventoryItem(Block.cobblestone.blockID))
        {
        	return;
        }
        par1ItemStack.damageItem(1, player);
    	EntityProjectile projectile = new EntityProjectile(worldObj, player, new ItemStack(Block.cobblestone), false);
    	projectile.damage = 3;
		projectile.punch = 1;
