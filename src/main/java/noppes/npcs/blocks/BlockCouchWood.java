...
public class BlockCouchWood extends BlockNpcContainerInterface{
    ...
    public boolean onBlockActivated(World par1World, int i, int j, int k, EntityPlayer player, int par6, float par7, float par8, float par9){
    	...
    	if(item == null || item.getItem() != Item.dyePowder)
    		return BlockChair.MountBlock(par1World, i, j, k, player);
    	int meta = par1World.getBlockMetadata(i, j, k);
    	if(meta >= 7)
    		j--;
    	TileColorable tile = (TileColorable) par1World.getBlockTileEntity(i, j, k);
    ...
    public void onBlockPlacedBy(World par1World, int par2, int par3, int par4, EntityLivingBase par5EntityLivingBase, ItemStack par6ItemStack)
    {
        ...
        l %= 4;
        TileCouchWood tile = (TileCouchWood) par1World.getBlockTileEntity(par2, par3, par4);
    	tile.rotation = l;
    	tile.color = 15 - par6ItemStack.getItemDamage();
        
        par1World.setBlockMetadataWithNotify(par2, par3, par4, par6ItemStack.getItemDamage(), 2);
    	updateModel(par1World, par2, par3, par4, tile);
        onNeighborBlockChange(par1World, par2 + 1, par3, par4, this.blockID);
        onNeighborBlockChange(par1World, par2 - 1, par3, par4, this.blockID);
        onNeighborBlockChange(par1World, par2, par3, par4 + 1, this.blockID);
        onNeighborBlockChange(par1World, par2, par3, par4 - 1, this.blockID);
    	updateModel(par1World, par2, par3, par4, tile);
    ...
    @Override  
    public void onNeighborBlockChange(World worldObj, int x, int y, int z, int block) {
    	if(worldObj.isRemote || block != this.blockID)
    		return;
    ...
    {
        return Block.planks.getIcon(p_149691_1_, meta);
    }
