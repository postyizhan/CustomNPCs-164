...
public class BlockCampfire extends BlockLightable{
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9){
    	...
    	if((item.getItem() == Item.flint || item.getItem() == Item.flintAndSteel) && unlitBlock() == this){
    	...
    	if(item.getItem().itemID == Block.sand.blockID && litBlock() == this){
        	super.onBlockActivated(world, x, y, z, player, par6, par7, par8, par9);
    	}
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random){
		...
		TileCampfire tile = (TileCampfire) world.getBlockTileEntity(x, y, z);
