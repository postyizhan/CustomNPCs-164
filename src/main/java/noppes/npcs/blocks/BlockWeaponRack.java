...
public class BlockWeaponRack extends BlockTrigger{
    ...
    public boolean onBlockActivated(World par1World, int i, int j, int k, EntityPlayer player, int side, float hitX, float hitY, float hitZ){
    	...
    	if(meta >= 7)
    		j--;
    	TileWeaponRack tile = (TileWeaponRack) par1World.getBlockTileEntity(i, j, k);
    	float hit = hitX;
    	if(tile.rotation == 2)
    		hit = 1 - hitX;
    	if(tile.rotation == 3)
	...
	@Override
    public void breakBlock(World world, int x, int y, int z, int block, int p_149749_6_){
		TileNpcContainer tile = (TileNpcContainer)world.getBlockTileEntity(x, y, z);
        if (tile == null)
        	return;
