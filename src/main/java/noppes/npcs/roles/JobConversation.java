...
public class JobConversation extends JobInterface{
	...
	public void writeEntityToNBT(NBTTagCompound compound) {
		compound.setCompoundTag("ConversationAvailability", availability.writeToNBT(new NBTTagCompound()));
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		NBTTagList nbttaglist = compound.getTagList("ConversationLines");
	...
	private boolean shouldRun() {
		...
		if(ticks > 0)
			return false;
		npcs.clear();
		List<EntityNPCInterface> list = npc.worldObj.getEntitiesWithinAABB(EntityNPCInterface.class, npc.boundingBox.expand(10, 20, 10));
		for(EntityNPCInterface npc : list){
			if(!npc.isKilled() && !npc.isAttacking() && names.contains(npc.getEntityName().toLowerCase()))
				npcs.put(npc.getEntityName().toLowerCase(), npc);
		}
		
		return names.size() == npcs.size();
	}
	...
	private void say(ConversationLine line) {
		List<EntityPlayer> inRange = npc.worldObj.getEntitiesWithinAABB(EntityPlayer.class, npc.boundingBox.expand(20D, 20D, 20D));
