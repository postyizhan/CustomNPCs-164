...
public class JobItemGiver extends JobInterface{
	...
	public void writeEntityToNBT(NBTTagCompound nbttagcompound) {
		...
		nbttagcompound.setCompoundTag("igJobInventory", inventory.getToNBT());
	}
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
    	...
    	lines = NBTTags.getStringList(nbttagcompound.getTagList("igLines"));
	...
	private boolean giveItems(EntityPlayer player){		
		PlayerItemGiverData data = PlayerDataController.instance.getItemGiverData(player);
		...
		if(isAllGiver()){
			...
			for(ItemStack is : items){
				if(!playerHasItem(player,is.itemID))
					toGive.add(is);
			}
		}
	...
	private int getDay(){
		return (int) (npc.worldObj.getWorldTime() / 24000L);
	}
	...
	public boolean aiShouldExecute() {
		ticks--;
		if(ticks > 0 || inventory.items.isEmpty())
			return false;
		ticks = 20;
