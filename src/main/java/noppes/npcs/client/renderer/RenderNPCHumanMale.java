...
public class RenderNPCHumanMale extends RenderNPCInterface
{
            ...
            float f, float f1)
    {
        ...
        super.doRenderLiving(npc, d, d3, d2, f, f1);
    ...
    protected void renderSpecials(EntityNPCInterface npc, float f)
    {
        ...
        if(itemstack2 != null)
        {
            ...
            if (itemstack2.getItem() instanceof ItemBlock && (is3D || RenderBlocks.renderItemIn3d(Block.blocksList[itemstack2.itemID].getRenderType())))
            {
                var6 = 0.5F;
            ...
            else if (itemstack2.itemID == Item.bow.itemID)
            {
                var6 = 0.625F;
            ...
            else if (Item.itemsList[itemstack2.itemID].isFull3D())
            {
                var6 = 0.625F;

                if (Item.itemsList[itemstack2.itemID].shouldRotateAroundWhenRendering())
                {
            ...
            this.renderManager.itemRenderer.renderItem(npc, itemstack2, 0);

            if (itemstack2.getItem().requiresMultipleRenderPasses())
            {
                for (int x = 1; x < itemstack2.getItem().getRenderPasses(itemstack2.getItemDamage()); x++)
                {
                    this.renderManager.itemRenderer.renderItem(npc, itemstack2, x);
                }
            }
