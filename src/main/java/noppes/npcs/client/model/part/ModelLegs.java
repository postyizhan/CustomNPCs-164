...
public class ModelLegs extends ModelScaleRenderer{
    ...
    public void setRotationAngles(float par1, float par2, float par3, float par4, float par5, float par6, Entity entity)
    {
		...
		if(part.type == 2){
	        ...
	        if(base.isSleeping(entity) || this.entity.currentAnimation == EnumAnimation.CRAWLING){
	        	rotationPointY = 12 * this.entity.legs.scaleY;
	        	rotationPointZ = 15 * this.entity.legs.scaleY;
    ...
    public void render(float par1)
    {
		if(!showModel || isHidden)
			return;
		ModelPartData part = entity.legParts;
		if(part.type < 0)
			return;
		GL11.glPushMatrix();
		if(part.type == 4)
    	...
    	if(bo){
	    	float red = (entity.legParts.color >> 16 & 255) / 255f;
	    	float green = (entity.legParts.color >> 8  & 255) / 255f;
	    	float blue = (entity.legParts.color & 255) / 255f;
