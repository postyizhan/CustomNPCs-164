...
public class GuiNPCInv extends GuiContainerNPCInterface2 implements ISliderListener, IGuiData
{
    ...
    {
        super.initGui();
        addLabel(new GuiNpcLabel(0,"inv.minExp", guiLeft + 118, guiTop + 18, 0x404040));
        addTextField(new GuiNpcTextField(0,this, fontRenderer, guiLeft + 108, guiTop + 29, 60, 20, npc.inventory.minExp + ""));
        getTextField(0).numbersOnly = true;
        getTextField(0).setMinMaxDefault(0, Short.MAX_VALUE, 0);
        
        addLabel(new GuiNpcLabel(1,"inv.maxExp", guiLeft + 118, guiTop + 52, 0x404040));
        addTextField(new GuiNpcTextField(1,this, fontRenderer, guiLeft + 108, guiTop + 63, 60, 20, npc.inventory.maxExp + ""));
        getTextField(1).numbersOnly = true;
        getTextField(1).setMinMaxDefault(0, Short.MAX_VALUE, 0);     

        addLabel(new GuiNpcLabel(2,"inv.npcInventory", guiLeft + 191, guiTop + 5, 0x404040));
        addLabel(new GuiNpcLabel(3,"inv.inventory", guiLeft + 8, guiTop + 101, 0x404040));
        ...
        for(int i = 0; i < 8; i++){
            ...
            GuiNpcSlider slider = new GuiNpcSlider(this, i, guiLeft + 211, guiTop + 15 + i * 22, null,
            		StatCollector.translateToLocal("inv.dropChance") + ": " + chance + "%", ((float)chance)/100);
            addSlider(slider);
        }
    }
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j)
    {
    	...
    	for(int id = 4; id <= 6;id++){
        	...
        	if(slot.getHasStack())
        		drawTexturedModalRect(guiLeft + slot.xDisplayPosition - 1, guiTop + slot.yDisplayPosition -1, 0, 0, 20, 20);
    	}
    }
	...
	public void save() {
    	...
    	NoppesUtil.sendData(EnumPacketType.MainmenuInvSave, npc.inventory.writeEntityToNBT(new NBTTagCompound()));
	}
