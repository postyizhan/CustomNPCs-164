...
public class GuiNpcManagePlayerData extends GuiNPCInterface2 implements IScrollData,GuiCustomScrollActionListener
{
    ...
    {
    	super(npc);
    	NoppesUtil.sendData(EnumPacketType.PlayerDataGet,selection);
    }
    ...
    public void initGui()
    {
        ...
        addLabel(new GuiNpcLabel(0,"All Players", guiLeft + 10, guiTop + 6, 0x404040));
    	...
    	addTextField(new GuiNpcTextField(0, this, fontRenderer, guiLeft + 4, guiTop + 193, 190, 20, search));
    ...
    public void initButtons(){
    	getButton(1).enabled = selection != EnumPlayerData.Players;
    	getButton(2).enabled = selection != EnumPlayerData.Quest;
    	getButton(3).enabled = selection != EnumPlayerData.Dialog;
    	getButton(4).enabled = selection != EnumPlayerData.Transport;
    	getButton(5).enabled = selection != EnumPlayerData.Bank;
    	getButton(6).enabled = selection != EnumPlayerData.Factions;
	...
	protected void actionPerformed(GuiButton guibutton)
    {
        if(guibutton.id == 0)
        {
        	if(selected != null){
        		if(selection == EnumPlayerData.Players)
        			NoppesUtil.sendData(EnumPacketType.PlayerDataRemove, selection,selectedPlayer,selected);
        		else
        			NoppesUtil.sendData(EnumPacketType.PlayerDataRemove, selection,selectedPlayer,data.get(selected));
        		data.clear();
        	}
        	selected = null;
        }
        ...
        {
        	if(selectedPlayer == null && guibutton.id != 1)
        		return;
        	selection = EnumPlayerData.values()[guibutton.id - 1];
        	initButtons();
        	scroll.clear();
        	data.clear();
        	NoppesUtil.sendData(EnumPacketType.PlayerDataGet, selection, selectedPlayer);
        	selected = null;
        }
    }
