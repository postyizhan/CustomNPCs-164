...
public class EntityProjectile extends Entity implements IProjectile {
    ...
    public EntityProjectile(World par1World, EntityLivingBase par2EntityLiving, ItemStack item, boolean isNPC)
    {
        ...
        setThrownItem(item);
        this.dataWatcher.updateObject(27, Byte.valueOf((byte) ((this.getItemId() == Item.arrow.itemID) ? 1 : 0)));
    ...
    public void onUpdate()
    {
        ...
        else
        {
	        ...
	        if (this.isRotating()) {
	        	...
	        	this.rotationPitch -= this.ticksInAir * spin * getSpeed();
	        }
	        ...
	        if (!this.dataWatcher.getWatchableObjectString(22).equals("")){
    ...
    protected void onImpact(MovingObjectPosition movingobjectposition)
    {
    	if (movingobjectposition.entityHit != null)
        {
            ...
            if (movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.getThrower()), damage))
            {
	            ...
	            if (this.isBlock())
	    		{
	        		this.worldObj.playAuxSFX(2001, (int) movingobjectposition.entityHit.posX, (int) movingobjectposition.entityHit.posY, (int) movingobjectposition.entityHit.posZ, this.getItemId());
	    		}
	            else if (!this.isArrow() && !this.sticksToWalls())
	    		{
			        for (int i = 0; i < 8; ++i)
			        {
			        	this.worldObj.spawnParticle("iconcrack_" + getItemId(), this.posX, this.posY, this.posZ, this.rand.nextGaussian() * 0.15D, this.rand.nextGaussian() * 0.2D, this.rand.nextGaussian() * 0.15D);
			        }
	    		}
...
//    			}
    		...
    		else
    		{
        		...
        		if (isBlock())
        		{
            		this.worldObj.playAuxSFX(2001, movingobjectposition.blockX, movingobjectposition.blockY, movingobjectposition.blockZ, this.getItemId());
        		}
        		else
        		{
    		        for (int i = 0; i < 8; ++i)
    	...
    	if (explosive)
    	{
    		...
    		{
    			this.worldObj.newExplosion((Entity)null, this.posX, this.posY, this.posZ, (float)this.explosiveRadius, this.effect == EnumPotionType.Fire, this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
    			if (this.explosiveRadius != 0 && (this.isArrow() || this.sticksToWalls())) this.setDead();
    		}		
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        {
            par1NBTTagCompound.setCompoundTag("Item", this.getItemDisplay().writeToNBT(new NBTTagCompound()));
        }
